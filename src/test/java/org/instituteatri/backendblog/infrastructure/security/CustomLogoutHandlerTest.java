...
class CustomLogoutHandlerTest {
    ...
    @Nested
    @DisplayName("FindTokenByValue Method Tests")
    ...
    @Test
    @DisplayName("Invalidate Token: Should set expired and revoked flags to true and save token")
    void invalidateToken_shouldSetExpiredAndRevokedToTrueAndSaveToken() {
        ...
        // Assert
        assertTrue(invalidateToken.isExpired(), "Token should be marked as expired");
        assertTrue(invalidateToken.isRevoked(), "Token should be marked as revoked");
        verify(tokenRepository).save(invalidateToken);
    }

    @Test
    @DisplayName("Send Unauthorized Response: Should set status and write message to response")
